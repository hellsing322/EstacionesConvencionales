...
class Calculos():
    ...
    def precipitacion_suma_diaria_171481h(self):
           ...
           fecha_dia_guardado=fecha_dia_siguiente.strftime("%Y-%m-%d %h:%m:%s")
           ...
           objeto.id_usuario=0
           objeto.fecha_ingreso=fecha_dia_guardado
           objeto.valor=valor
           objeto.validado=False
           objeto.save()

           
    def calculo_evaporacion_inst_614161h(self):
        ...
        fecha_dia_guardado=fecha_dia_siguiente.strftime("%Y-%m-%d %h:%m:%s")
        id_estacion=T1714161H.objects.filter(fecha_ingreso__range=(fecha_actual+" 13:00:00",fecha_actual+" 13:59:59")).values('id_estacion')
        valorprec =T1714161H.objects.filter(fecha_ingreso__range=(fecha_actual+" 07:00:00",fecha_actual+" 07:59:59")).values('valor')
        valor_agua_sacada =T614161H.objects.filter(fecha_ingreso__range=(fecha_actual+" 07:00:00",fecha_actual+" 07:59:59")).values('valor')
        valor_agua_añadida =T775161D.objects.filter(fecha_ingreso__range=(fecha_actual+" 07:00:00",fecha_actual+" 07:59:59")).values('valor')
        

        for a in id_estacion:
                    estacion=a['id_estacion']

        if valorprec.exists()==True :
               for a in valorprec:
                    valor1=a['valor']
        else:
        ...
        if valor_agua_sacada.exists()==True :
               for a in valor_agua_sacada:
                    valor2=a['valor']
        else:
                valor2='no existe valor 13 horas'
                return (valor2)

        if valor_agua_añadida.exists()==True :
               for a in valor_agua_añadida:
                    valor3=a['valor']
        else:
